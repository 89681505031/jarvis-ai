...
from user_database import UserDatabase, db as user_db
from firebase_sync import FirebaseSync, firebase_sync, FIREBASE_OK

# Загружаем API ключи из БД
GIGACHAT_AUTH_KEY = user_db.get_api_key('gigachat') or os.environ.get('GIGACHAT_AUTH_KEY', '')
FISH_AUDIO_API_KEY = user_db.get_api_key('fish_audio') or os.environ.get('FISH_AUDIO_API_KEY', '')
GIGACHAT_OK = bool(GIGACHAT_AUTH_KEY)
...
class JARVISUltimate(tk.Tk):
    def __init__(self):
        ...
        self.fish_api_key = os.environ.get('FISH_AUDIO_API_KEY', FISH_AUDIO_API_KEY)
        ...
        # === ПРОВЕРКА АВТО-ВХОДА ===
        saved_user_id = user_db.get_api_key('firebase_user_id')
        if saved_user_id:
            log.info("🔍 Найден сохранённый user_id, проверяю авто-вход...")
            self.after(1000, lambda: self._auto_login(saved_user_id))
        else:
            log.info("🔍 Нет сохранённого user_id, показываю регистрацию")
            self.after(1000, self.show_registration_window)
    ...
    def _init_reference_ui(self):
        ...
        ModernButton(sidebar, text="👤  PROFILE", command=self.open_profile, bg="#0d111a",
                     fg="#38bdf8", activebackground="#1a2533",
    ...
    def open_profile(self):
        ...
        profile_win.geometry("500x500")
        profile_win.configure(bg=self.panel_bg)
        profile_win.grab_set()
        
        tk.Label(profile_win, text="👤 ПРОФИЛЬ ПОЛЬЗОВАТЕЛЯ", font=("Segoe UI", 13, "bold"), 
                 bg=self.panel_bg, fg=self.accent_color).pack(pady=(15, 10))
        
        user = user_db.get_user()
        if user:
            info_frame = tk.Frame(profile_win, bg=self.panel_bg)
            info_frame.pack(fill=tk.X, padx=20, pady=10)
            
            tk.Label(info_frame, text=f"Имя: {user['name']}", font=("Segoe UI", 11), 
                     bg=self.panel_bg, fg="#e2e8f0").pack(anchor="w", pady=5)
            tk.Label(info_frame, text=f"Дата рождения: {user.get('birth_date', 'N/A')}", font=("Segoe UI", 11), 
                     bg=self.panel_bg, fg="#94a3b8").pack(anchor="w", pady=5)
            
            # API ключи
            api_frame = tk.Frame(profile_win, bg=self.panel_bg)
            api_frame.pack(fill=tk.X, padx=20, pady=20)
            
            tk.Label(api_frame, text="🔑 API КЛЮЧИ", font=("Segoe UI", 11, "bold"), 
                     bg=self.panel_bg, fg=self.accent_color).pack(anchor="w", pady=(0, 10))
            
            gigachat_key = user_db.get_api_key('gigachat')
            fish_key = user_db.get_api_key('fish_audio')
            
            tk.Label(api_frame, text=f"GigaChat: {'✅ Настроен' if gigachat_key else '❌ Не настроен'}", 
                     font=("Segoe UI", 10), bg=self.panel_bg, fg="#10b981" if gigachat_key else "#ef4444").pack(anchor="w", pady=2)
            tk.Label(api_frame, text=f"Fish Audio: {'✅ Настроен' if fish_key else '❌ Не настроен'}", 
                     font=("Segoe UI", 10), bg=self.panel_bg, fg="#10b981" if fish_key else "#ef4444").pack(anchor="w", pady=2)
            
            # Firebase статус
            if FIREBASE_OK and firebase_sync.is_connected:
                tk.Label(api_frame, text=f"Firebase: ✅ Подключено", 
                         font=("Segoe UI", 10), bg=self.panel_bg, fg="#10b981").pack(anchor="w", pady=2)
            
            # Кнопка выход
            def logout():
                if messagebox.askyesno("Выход", "Выйти со всех устройств?"):
                    firebase_sync.force_logout_all()
                    user_db.set_api_key('firebase_user_id', None)
                    profile_win.destroy()
                    self.show_login_window()
            
            ModernButton(profile_win, text="🚪 ВЫЙТИ", command=logout, 
                         bg="#dc2626", fg="#fff", padx=20, pady=8).pack(pady=20)
        
        def close():
            profile_win.destroy()
        
        ModernButton(profile_win, text="ЗАКРЫТЬ", command=close, 
                     bg="#334155", fg="#fff", padx=20, pady=8).pack(pady=10)
    ...
    def open_settings(self):
        ...
        tk.Label(gigachat_frame, text="GigaChat API:", font=("Segoe UI", 9), bg=self.panel_bg, fg="#94a3b8", anchor="w").pack(anchor="w", pady=(0, 5))
        
        self.gigachat_key_var = tk.StringVar(value=user_db.get_api_key('gigachat') or "")
        tk.Entry(gigachat_frame, textvariable=self.gigachat_key_var, font=("Segoe UI", 9), bg="#0f172a", fg="#e2e8f0", relief=tk.SOLID, bd=1, show="*").pack(fill=tk.X, pady=(0, 5))
        
        def save_gigachat_key():
            new_key = self.gigachat_key_var.get().strip()
            if new_key:
                user_db.set_api_key('gigachat', new_key)
                messagebox.showinfo("Успех", "API ключ GigaChat сохранён!")
            else:
                messagebox.showwarning("Внимание", "Ключ не может быть пустым")
        
        ModernButton(gigachat_frame, text="💾 Сохранить GigaChat", command=save_gigachat_key, bg="#059669", fg="#fff", padx=10, pady=5).pack(fill=tk.X)
        
        # Fish Audio API Key
        fish_frame2 = tk.Frame(api_frame, bg=self.panel_bg)
        fish_frame2.pack(fill=tk.X, pady=(10, 0))
        
        tk.Label(fish_frame2, text="Fish Audio API:", font=("Segoe UI", 9), bg=self.panel_bg, fg="#94a3b8", anchor="w").pack(anchor="w", pady=(0, 5))
        
        self.fish_api_key_var = tk.StringVar(value=user_db.get_api_key('fish_audio') or "")
        tk.Entry(fish_frame2, textvariable=self.fish_api_key_var, font=("Segoe UI", 9), bg="#0f172a", fg="#e2e8f0", relief=tk.SOLID, bd=1, show="*").pack(fill=tk.X, pady=(0, 5))
        
        def save_fish_key():
            new_key = self.fish_api_key_var.get().strip()
            if new_key:
                user_db.set_api_key('fish_audio', new_key)
                messagebox.showinfo("Успех", "API ключ Fish Audio сохранён!\nПерезапустите JARVIS для применения.")
            else:
                messagebox.showwarning("Внимание", "Ключ не может быть пустым")
        
        ModernButton(fish_frame2, text="💾 Сохранить Fish Audio", command=save_fish_key, bg="#7c3aed", fg="#fff", padx=10, pady=5).pack(fill=tk.X)
...
"• Время и дата: какой час, какая дата, календарь\n"
    ...
    def show_login_window(self):
        ...
        tk.Label(login_win, text="🔐 J.A.R.V.I.S.", font=("Segoe UI", 20, "bold"), bg="#1e293b", fg="#38bdf8").pack(pady=(30, 10))
        tk.Label(login_win, text="Вход в аккаунт", font=("Segoe UI", 11), bg="#1e293b", fg="#94a3b8").pack(pady=(0, 25))
        
        tk.Frame(login_win, bg="#334155", height=2).pack(fill=tk.X, padx=40, pady=10)
        
        tk.Label(login_win, text="Email:", font=("Segoe UI", 9), bg="#1e293b", fg="#cbd5e1").pack(anchor="w", padx=40, pady=(5, 2))
        ...
        toggle_btn = tk.Button(login_win, text="👁", font=("Segoe UI", 10), bg="#334155", fg="#e2e8f0", command=toggle_password, relief=tk.FLAT, width=3)
        ...
        def login():
            ...
            if success:
                user_db.set_api_key('firebase_user_id', firebase_sync.user_id)
        ...
        ModernButton(login_win, text="🔓 ВХОД", font=("Segoe UI", 12, "bold"), bg="#2563eb", fg="#fff", command=login, padx=30, pady=12).pack(pady=(10, 5))
        
        register_btn = tk.Label(login_win, text="Нет аккаунта? Зарегистрироваться", font=("Segoe UI", 9), bg="#1e293b", fg="#38bdf8", cursor="hand2")
    ...
    def show_registration_window(self):
        ...
        tk.Label(reg_win, text="🤖 J.A.R.V.I.S.", font=("Segoe UI", 20, "bold"), bg="#1e293b", fg="#38bdf8").pack(pady=(30, 10))
        tk.Label(reg_win, text="Персональный ИИ-ассистент", font=("Segoe UI", 11), bg="#1e293b", fg="#94a3b8").pack(pady=(0, 25))
        
        tk.Frame(reg_win, bg="#334155", height=2).pack(fill=tk.X, padx=40, pady=10)
        
        tk.Label(reg_win, text="👤 ЛИЧНЫЕ ДАННЫЕ", font=("Segoe UI", 11, "bold"), bg="#1e293b", fg="#38bdf8").pack(pady=(15, 8))
        ...
        tk.Label(reg_win, text="🔑 API КЛЮЧИ (опционально)", font=("Segoe UI", 11, "bold"), bg="#1e293b", fg="#38bdf8").pack(pady=(0, 8))
        ...
        def save_registration():
            ...
            user_db.set_user(name, "01", "января", 2000)
            
            # Сохраняем API ключи
            gigachat_key = gigachat_entry.get().strip()
            if gigachat_key:
                user_db.set_api_key('gigachat', gigachat_key)
            
            fish_key = fish_entry.get().strip()
            if fish_key:
                user_db.set_api_key('fish_audio', fish_key)
            
            reg_win.destroy()
            self.after(100, self.show_welcome)
        
        ModernButton(reg_win, text="🚀 НАЧАТЬ РАБОТУ", font=("Segoe UI", 12, "bold"), bg="#2563eb", fg="#fff", command=save_registration, padx=30, pady=12).pack(pady=(15, 10))
    
    def _auto_login(self, user_id):
        """Автоматический вход с сохранённой сессией"""
        try:
            success, user_data, session_id = firebase_sync.auto_login(user_id)
