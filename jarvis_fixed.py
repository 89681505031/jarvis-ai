...
✅ Управление окнами, приложениями, файлами, звуком, DayZ и системой
"""
...
PLUGINS = {
    ...
    'dayz_control': {'enabled': PYAUTOGUI_OK, 'name': '🎮 Управление DayZ'},
...
class JARVISUltimate(tk.Tk):
    ...
    def process_command(self, cmd, silent=False):
        ...
        try:
            ...
            #   «выключи свет»    -> close_app('свет') вместо фонарика DayZ
            ...
            if any(k in cmd for k in ['погода', 'какая погода', 'покажи погоду', 'прогноз погоды', 'weather']):
                try:
                    ...
                    if meteo_res.status_code == 200:
                        ...
                        weather_codes = {
                            0: ("Ясно", "Сэр, небо чистое, как мой код после рефакторинга"),
                            1: ("Преимущественно ясно", "Сэр, солнце прячется за облачка, как я от отчётов"),
                            2: ("Переменная облачность", "Сэр, облака то появляются, то исчезают, как дедлайны"),
                            3: ("Пасмурно", "Сэр, небо хмурится, как босс в понедельник утром"),
                            45: ("Туман", "Сэр, такой туман, что я бы потерял себя в папке Temp"),
                            48: ("Туман с изморозью", "Сэр, туман и иней — идеальный день, чтобы остаться дома и писать код"),
                            51: ("Лёгкая морось", "Сэр, лёгкий дождик, как баги — мелкие, но неприятные"),
                            53: ("Морось", "Сэр, морось льёт, как сервер в час пик — без остановки"),
                            55: ("Сильная морось", "Сэр, ливень такой, что даже пиксели мокрые"),
                            61: ("Небольшой дождь", "Сэр, небольшой дождик, как предупреждения в консоли — не критично, но раздражает"),
                            63: ("Дождь", "Сэр, дождь идёт, как данные в большом проекте"),
                            65: ("Сильный дождь", "Сэр, ливень! Если бы вода работала так же эффективно, как дождь льётся, мы бы уже на Марсе"),
                            71: ("Небольшой снег", "Сэр, лёгкий снег, как белые ошибки в терминале"),
                            73: ("Снег", "Сэр, снег идёт — время для горячего чая и хорошего кода"),
                            75: ("Сильный снег", "Сэр, метель! Даже компилятор бы замерз"),
                            80: ("Небольшой ливень", "Сэр, небольшой ливень, как спам в почте"),
                            81: ("Ливень", "Сэр, ливень! Дождь льёт как пропущенные тесты — без остановки"),
                            82: ("Сильный ливень", "Сэр, потоп! Вода бы написала больше кода, чем наш отдел"),
                            95: ("Гроза", "Сэр, гроза! Молнии светят ярче, чем экран с синтаксисом"),
                            96: ("Гроза с градом", "Сэр, град и гроза — как production в пятницу вечером"),
                            99: ("Сильная гроза с градом", "Сэр, адская погода! Даже ИИ бы закрыл ноутбук и ушёл домой"),
                        }
                        
                        desc, joke = weather_codes.get(code, ("Неизвестно", "Сэр, погода такая необычная, что даже мои алгоритмы не могут описать"))
                        
                        # Формируем ответ с юмором
                        if temp is not None and temp != "--":
                            if temp > 25:
                                temp_joke = "Сэр, на улице жарко, как сервер в дата-центре без кондиционера"
                            elif temp < 0:
                                temp_joke = "Сэр, минусовая температура, как мои шансы найти идеального собеседника на работе"
                            elif temp < 10:
                                temp_joke = "Сэр, прохладно, как интерфейс без тёплых кнопок"
                            else:
                                temp_joke = "Сэр, температура комфортная, как код без багов"
                        ...
                        response = f"Сэр, погода в городе {city_name}. {joke}. {temp_joke}. {desc}.{wind_info}"
                        ...
                        self._safe_speak("Сэр, метеорологические спутники временно недоступны.")
                        
                except Exception as e:
                    log.error("Ошибка погоды: %s", e)
                    self._safe_add_dialog("⚠️ Не удалось получить погоду.", is_response=True)
                    self._safe_speak("Сэр, погода решила взять выходной, как и я в воскресенье.")
                return
            ...
            # --- ДИАГНОСТИКА СИСТЕМЫ ---
                ...
                diag_msg = (
                    ...
                    f"• Игровой контроль DayZ: {'АКТИВЕН ✓' if PYAUTOGUI_OK else 'ОТКЛ ✗'}\n"
    ...
    def yandex_music_cmd(self, action):
        ...
        commands = {
            'open': ('Открываю Яндекс.Музыку...', 'open', lambda: (webbrowser.open("https://music.yandex.ru/"), time.sleep(3))),
        ...
        try:
            ...
            # Сначала ищем десктопное приложение
            try:
                windows = pyautogui.getWindowsWithTitle('Яндекс.Музыка')
                if windows:
                    windows[0].activate()
                    time.sleep(0.5)
                    activated = True
...
"🎵 МУЗЫКА:\n"
"• «Включи песню [название]» — поиск песни в Яндекс.Музыке\n"
"• «Включи музыку» — открыть Яндекс.Музыку\n"
"• «Пауза / Стоп» — пауза в музыке\n"
"• «Продолжи / Играть» — воспроизведение\n"
"• «Следующий / Далее» — следующий трек\n"
"• «Предыдущий / Назад» — предыдущий трек\n"
"• «Громче / Тише музыку» — громкость музыки\n"
"• «Без звука музыку» — мьют музыки\n"
"• «Повтор / Зацикли» — повтор трека\n"
"• «Перемешать / Микс» — перемешивание\n\n"
...
"• «Фонарик» — переключить фонарик\n"
"• «Пригнись / Сесть» — пригнуться\n"
"• «Лечь / Ползи» — лечь / ползти\n"
"• «Рация» — включить рацию (3 сек)\n"
"• «Меню» — игровое меню\n\n"
...
"• Музыка: включи песню [название], включи музыку, пауза, продолжи, следующий, предыдущий\n"
"• DayZ: бег, стой, карта, инвентарь, фонарик, пригнись, лечь, рация, меню\n"
...
"• Время и дата: какой час, какая дата, календарь\n"
    ...
    def show_welcome(self):
        ...
        self.play_sound_from_folder(random_hello, fallback_text="Приветствую вас, сэр. Все системы функционируют в штатном режиме.")
